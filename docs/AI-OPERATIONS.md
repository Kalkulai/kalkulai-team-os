...
- Unstrukturierte Hermes-Expense-Inputs zuerst in Beleg-/Vendor-/Datum-/Betrag-/Zahler-Fakten zerlegen. Nur Fakten uebernehmen, die im Input oder in einem erreichbaren Beleg/Produkt-Link stehen; fehlende Pflichtfelder nicht raten, sondern als `unclear`/kurze `note` markieren oder zur Klaerung offenlassen.
...
- Konservative Foerderlogik: Hardware, Cloud und projektnahe Tools koennen `fundable`/`sachmittel` sein, wenn Beleg und Projektbezug klar sind. Coaching gehoert in `coaching`. Stipendien gehoeren in `stipend`. Notar-/GmbH-Gruendungskosten sind `non_fundable`. Marketing/Ads und unklare Legal-Faelle bleiben `unclear`, bis die Foerderfaehigkeit sauber belegt ist. Bei gemischten Warenkoerben oder unsicherem EXIST-Bezug immer den konservativeren Wert waehlen und die Unsicherheit in `note` dokumentieren.
- Produkt-Link-Handling: Wenn ein Produkt-/Beleg-Link wegen Block, Paywall, 403/429/503 oder Timeout nicht erreichbar ist, keine Produktdetails, Preise, Anbieter oder Foerderklassifikation halluzinieren. Schreibe nur die aus Input/Beleg bekannten Daten, setze Klassifikation konservativ auf `unclear`, und notiere den Blocker inkl. Statuscode/Fehler in `note` oder `source_message`.
